  ...
  try {
    ...
    const averageExpense =
      data.length > 0 && data[0].count > 0
        ? data[0].totalExpense / data[0].count
        : 0;

    return withCors(NextResponse.json({ success: true, data: [{ totalExpense: averageExpense }] }), req);
  } catch (error: any) {
    console.error("Error in average-expanse:", error);
    return withCors(NextResponse.json(
