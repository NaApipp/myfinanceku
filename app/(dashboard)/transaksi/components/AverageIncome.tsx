...
interface AverageIncome {
  totalIncome: number;
}

export default function AverageIncome() {
  const [loading, setLoading] = useState(true);
  const [average, setAverage] = useState<AverageIncome[]>([]);

  useEffect(() => {
    const fetchAccounts = async () => {
      try {
        const response = await fetch("/api/transaksi/average-income");
  ...
  const totalIncome = average.reduce(
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 rounded-[32px] border border-gray-100 dark:border-white/5 shadow-sm p-6 flex items-center justify-between hover:shadow-md transition-all duration-300 group">
      <div>
        ...
        <h4 className="text-2xl font-black text-emerald-600 dark:text-emerald-400 tracking-tight transition-transform duration-300 group-hover:scale-[1.02] origin-left">
