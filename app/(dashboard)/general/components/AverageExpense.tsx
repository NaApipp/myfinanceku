...
export default function AverageExpense() {
  const [loading, setLoading] = useState(true);
  const [average, setAverage] = useState<AverageExpense[]>([]);

  useEffect(() => {
    const fetchAccounts = async () => {
      try {
        const response = await fetch("/api/transaksi/average-expanse");
  ...
  const totalExpense = average.reduce(
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 rounded-[32px] border border-gray-100 dark:border-white/5 shadow-sm p-6 flex items-center justify-between hover:shadow-md transition-all duration-300 group">
      <div>
        ...
        <h4 className="text-2xl font-black text-rose-600 dark:text-rose-400 tracking-tight transition-transform duration-300 group-hover:scale-[1.02] origin-left">
      ...
      <div className="w-12 h-12 bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 rounded-2xl flex items-center justify-center border border-rose-100/50 dark:border-rose-500/10 shadow-sm transition-transform duration-300 group-hover:scale-110">
