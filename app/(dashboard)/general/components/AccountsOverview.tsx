  ...
  return (
    <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm p-6 flex flex-col h-full">
      <div className="flex items-center justify-between mb-6">
        <h3 className="font-bold text-lg text-gray-900">Akun & Kartu</h3>
      ...
      <div className="space-y-3 flex-1">
        {accounts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 text-center">
            <p className="text-sm text-gray-500 font-medium">Belum ada akun terdaftar</p>
          </div>
        ) : (
          accounts.slice(0, 4).map((acc) => (
            <div key={acc.idAccount} className="flex items-center justify-between p-4 bg-gray-50/50 hover:bg-gray-50 rounded-2xl transition-colors border border-transparent hover:border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span className="text-sm font-bold text-gray-900">{acc.nama_akun}</span>
                  <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">{acc.type_asset}</span>
                </div>
              </div>
              <div className="text-right">
                <span className="text-sm font-black text-gray-900">{formatCurrency(acc.saldo_awal)}</span>
              </div>
            </div>
          ))
        )}
        {accounts.length > 4 && (
          <p className="text-center text-xs text-gray-400 font-medium pt-2">+{accounts.length - 4} akun lainnya</p>
        )}
      </div>
    </div>
  );
